...
CSS: display:flex | justify-content:center | align-items:center'};\n    if(id==='css-flexbox-playground') return {output:'.container {\n  display: flex;\n  flex-direction: row;\n  justify-content: center;\n  align-items: center;\n  flex-wrap: wrap;\n  gap: 1rem;\n}'};
    if(id==='css-grid-generator') { const p=text.split(/[ ,x]+/).map(Number).filter(Number.isFinite); const cols=p[0]||3, rows=p[1]||2; return {output:'.grid {\n  display: grid;\n  grid-template-columns: repeat('+cols+', minmax(0, 1fr));\n  grid-template-rows: repeat('+rows+', auto);\n  gap: 1rem;\n}'}; }
    if(id==='css-clamp-calculator') { const p=text.split(/[ ,]+/).map(Number).filter(Number.isFinite); const min=p[0]??16,max=p[1]??32; return {output:'font-size: clamp('+min+'px, calc('+min+'px + '+(max-min)+' * ((100vw - 320px) / 1120)), '+max+'px);'}; }
    if(id==='css-badge-chip-generator') return {output:'.badge {\n  display: inline-flex;\n  align-items: center;\n  padding: 0.25rem 0.65rem;\n  border-radius: 9999px;\n  font-size: 0.75rem;\n  font-weight: 600;\n  background: #312e81;\n  color: white;\n}'};
    if(id==='css-button-styles-library') return {output:'.btn {\n  display: inline-flex;\n  align-items: center;\n  justify-content: center;\n  padding: .65rem 1rem;\n  border: 0;\n  border-radius: .65rem;\n  font-weight: 600;\n  cursor: pointer;\n  transition: transform .15s ease, opacity .15s ease;\n}\n.btn:hover { transform: translateY(-1px); opacity: .92; }'};
    if(id==='html-table-generator') { const rows=parseCsv(input); if(!rows.length) throw new Error('Enter CSV-like rows first.'); const head=rows[0].map(x=>'<th>'+escapeHtml(x)+'</th>').join(''); const body=rows.slice(1).map(row=>'<tr>'+row.map(x=>'<td>'+escapeHtml(x)+'</td>').join('')+'</tr>').join('\n'); return {output:'<table>\n  <thead><tr>'+head+'</tr></thead>\n  <tbody>\n'+body+'\n  </tbody>\n</table>'}; }
    if(id==='web-manifest-generator') { const name=text||'Coding Super Hub'; return {output:JSON.stringify({name,short_name:name.slice(0,12),start_url:'/',display:'standalone',theme_color:'#0f172a',background_color:'#020617',icons:[]},null,2)}; }
